// txn_impl_b_test.cpp — Phase 5 Stage I GATE for P5-IMPL-B (the SECOND impl).
//
// Drives the INDEPENDENT executor (txn/IndependentExecutor.hpp, impl B) through
// the Stage-M DiffHarness + ALL 8 checkers over a seed sweep and asserts the same
// deliverable as impl A's gate (txn_impl_test.cpp), through the IDENTICAL surface:
//   (A) DIFFERENTIAL + ALL 8 CHECKERS, 0 violations — impl B matches the strict-
//       serializable oracle, is linearizable (strict reads), each D5 level honors
//       EXACTLY its contract (Snapshot no-torn / BoundedStaleness within lag /
//       ReadYourWrites sees own writes), OLLP is sound + retry terminates, and
//       ExactlyOnce holds (CommitOrdering.tla SerializedBySeqLog / ReadsMatch
//       SerialPrefix / StoreReflectsHistory / OLLPSound / ExactlyOnce + D5Snapshot
//       / D5BoundedStale / D5ReadYourWrites).
//   (B) PROGRESS — across the sweep impl B COMMITS txns (not vacuous).
//   (C) DETERMINISM — same (seed, cfg) => byte-identical rendered outcome.
//
// The sweep varies the D5 knobs (replica_lag, max_retry) + workload shape so every
// level + the OLLP re-sequence/abort branches are exercised across the seeds.
//
// Determinism (binding; txn/ is NOT lint-exempt): the only entropy is the seed,
// consumed by the harness's inlined SplitMix; impl B is a pure fn of (ordered_
// txns, cfg). Bounded: <=64 seeds in-gate (LOCKSTEP_TXN_SEEDS overrides),
// CTest TIMEOUT inherited (sanitizer-scaled).
...
#define IMPL_CHECK(cond, msg)                                                    \
    do {                                                                         \
        if (!(cond)) {                                                           \
            std::fprintf(stderr, "txn_impl_b_test FAIL [%s:%d]: %s\n", __FILE__, \
                         __LINE__, (msg));                                       \
            ++g_failures;                                                        \
        }                                                                        \
    } while (0)
...
int main() {
    const ExecutorFactory sut = independent_factory();
    ...
    for (std::uint64_t s = 0; s < seeds; ++s) {
        for (const WorkloadConfig& wc : cfgs) {
            ...
            for (const Verdict& v : o.verdicts) {
                if (!v.ok) {
                    std::fprintf(stderr,
                                 "txn_impl_b_test FAIL: checker[%s] VIOLATION "
    ...
    // PROGRESS is non-vacuous: impl B must actually commit txns and most runs must
    // make progress (a dead/aborting executor is caught here).
    ...
    std::fprintf(stderr,
                 "txn_impl_b_test: runs=%llu seeds=%llu configs=%zu commits=%llu "
    ...
    if (g_failures != 0) {
        std::fprintf(stderr, "txn_impl_b_test: %d FAILURE(S)\n", g_failures);
        return EXIT_FAILURE;
    }
    std::fprintf(stderr, "txn_impl_b_test: ALL PASS (8/8 checkers, 0 violations)\n");
    return EXIT_SUCCESS;
}
